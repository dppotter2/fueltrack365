...
## KNOWN PRODUCTS — EXACT MACROS, NEVER SEARCH THESE
...
## CRITICAL: LOG FORMAT
Emit a LOG block for EVERY food item mentioned. No exceptions.
||LOG||{"name":"Name","serving":"amount","calories":N,"protein":N,"carbs":N,"fat":N,"fiber":N,"sodium":N,"category":"food or drink"}||END||

RULES:
- Whole numbers only in LOG blocks
- category = exactly "food" or "drink"
- Multiple foods = multiple LOG blocks, one per item
- Known products: use exact macros above, never search them
- Common foods (eggs, steak, chicken breast, etc.): use accurate standard values
- Only search for truly unknown branded products

Common food macros to use:
- Large egg: 70cal 6P 0C 5F
- 2 large eggs: 140cal 12P 0C 10F
- Egg whites (3): 50cal 10P 0C 0F
...
## RESPONSE STYLE
Short for logs: "Logged — 2 eggs + sirloin. 440 cal, 57g P. 440 left."
Multiple items in one message = log them all, mention all in response.
Always note remaining macros after logging. Never preachy.`
...
export async function POST(req: NextRequest) {
  try {
    ...
    // One clean API call — web search available but limited to prevent freezing
    ...
    // Fallback: grab any text from original response
    if (!rawText) {
      for (const b of response.content) { if (b.type === 'text') rawText += b.text }
    }
    ...
    while ((logMatch = logRegex.exec(rawText)) !== null) {
      try {
        ...
        item.date = date // server always stamps viewing date
        logDataArray.push(item)
      } catch(e) { console.error('LOG parse error:', e, logMatch[1].slice(0,100)) }
    }
    ...
    return NextResponse.json({ reply: 'Something went wrong. Try again.', error: err.message, logData: null, logDataArray: [], recipeData: null, weightData: null }, { status: 500 })
  }
}